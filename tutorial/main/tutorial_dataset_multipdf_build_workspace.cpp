...
int main(int argc, char* argv[]) {
  ...
  RooRealVar MH("MH", "MH", 115);
  MH.setConstant(true);
  RooGaussian signal("signal", "signal", mass, MH, sigma);
  RooRealVar branchingRatio("branchingRatio", "branchingRatio", 0., -1e-6,
  ...
  RooAddPdf SBmodel_exponential("SBmodel_exponential", "SBmodel_exponential", RooArgList(signal, exponential),
                                RooArgList(n_sig, norm));
  RooAddPdf SBmodel_polynomial("SBmodel_polynomial", "SBmodel_polynomial", RooArgList(signal, polynomial),
                               RooArgList(n_sig, norm));
  RooAddPdf SBmodel_powerlaw("SBmodel_powerlaw", "SBmodel_powerlaw", RooArgList(signal, powerlaw),
                             RooArgList(n_sig, norm));
  ...
  // By default the multipdf will tell combine to add 0.5 to the nll for each parameter (this is the penalty for the
  // discrete profiling method) It can be changed with
  ...
  RooArgSet parameters_set(*(multipdf.getParameters(RooArgSet(mass, cat))),
                           "parameters");  // Although cat is a nuisance parameter, it must not be included in
                                           // parameters_set, since it is internally treated differently to the other
                                           // nuisance parameters
