...
void PullPlotter::plotPullsCanvas(std::vector<TString>& observables, int currentid, int maxid, int nObs) {
  ...
  if (currentid == maxid) {
    ...
    t3->SetTextAlign(12);
    t3->SetTextFont(133);
    t3->AddText(Form("#chi^{2}/(nObs-nPar) = %.2f/(%i-%i), P(#chi^{2},%i)=%.3f", chi2, nObs, nPar, nObs - nPar,
                     TMath::Prob(chi2, nObs - nPar)))
        ->SetTextSize(25);
    t3->Draw();
  }
...
void PullPlotter::plotPulls() {
  ...
  for (int i = obsOrder.size() - 1; i >= 0; i--) {
    ...
    if (arg->verbose) {
      printf("%30s: obs = %9.6f +/- %9.6f, th = %9.6f\n", pObs->GetName(), pObs->getVal(), pObs->getError(),
             pTh->getVal());
    }
  ...
  for (int i = 0; i < observables.size(); i++) {
    ...
    if (observablesChunk.size() % 10 == 0) {
      plotPullsCanvas(observablesChunk, ceil(i / 10.), ceil(observables.size() / 10.), observables.size());
      observablesChunk.clear();
    }
  }
  if (observables.size() % 10 != 0) {
    plotPullsCanvas(observablesChunk, ceil(observables.size() / 10.), ceil(observables.size() / 10.),
                    observables.size());
  }
