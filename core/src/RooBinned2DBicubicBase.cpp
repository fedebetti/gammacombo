...
RooBinned2DBicubicBase<BASE>::RooBinned2DBicubicBase(const RooBinned2DBicubicBase<BASE>& other, const char* name)
    : BASE(other, name), x("x", this, other.x), y("y", this, other.y), nBinsX(other.nBinsX), nBinsY(other.nBinsY),
      binSizeX(other.binSizeX), binSizeY(other.binSizeY), xmin(other.xmin), xmax(other.xmax), ymin(other.ymin),
      ymax(other.ymax), coeffs(other.coeffs) {}
...
template <class BASE>
inline double RooBinned2DBicubicBase<BASE>::histcont(const TH2& h, int xbin, int ybin) const {
  return (0 <= xbin && xbin < nBinsX - 1 && 0 <= ybin && ybin < nBinsY - 1) ? h.GetBinContent(1 + xbin, 1 + ybin) : 0.;
}

template <class BASE>
inline double RooBinned2DBicubicBase<BASE>::dhistdx(const TH2& h, int xbin, int ybin) const {
  return 0.5 * (histcont(h, xbin + 1, ybin) - histcont(h, xbin - 1, ybin));
}

template <class BASE>
inline double RooBinned2DBicubicBase<BASE>::dhistdy(const TH2& h, int xbin, int ybin) const {
  return 0.5 * (histcont(h, xbin, ybin + 1) - histcont(h, xbin, ybin - 1));
}

template <class BASE>
inline double RooBinned2DBicubicBase<BASE>::d2histdxdy(const TH2& h, int xbin, int ybin) const {
  return 0.5 * (histcont(h, xbin - 1, ybin - 1) - histcont(h, xbin + 1, ybin - 1) + histcont(h, xbin + 1, ybin + 1) -
                histcont(h, xbin - 1, ybin + 1));
}

template <class BASE>
RooBinned2DBicubicBase<BASE>::RooBinned2DBicubicBase(const char* name, const char* title, const TH2& h,
                                                     RooAbsReal& xvar, RooAbsReal& yvar)
    : BASE(name, title), x("x", "x", this, xvar), y("y", "y", this, yvar), nBinsX(1 + h.GetNbinsX()),
      nBinsY(1 + h.GetNbinsY()), binSizeX(h.GetXaxis()->GetBinWidth(1)), binSizeY(h.GetYaxis()->GetBinWidth(1)),
      xmin(h.GetXaxis()->GetBinCenter(1) - binSizeX), xmax(h.GetXaxis()->GetBinCenter(nBinsX - 1) + binSizeX),
      ymin(h.GetYaxis()->GetBinCenter(1) - binSizeY), ymax(h.GetYaxis()->GetBinCenter(nBinsY - 1) + binSizeY),
      coeffs(CoeffRecLen * nBinsX * nBinsY) {
  const TAxis *xaxis = h.GetXaxis(), *yaxis = h.GetYaxis();
  // verify that all bins have same size
  for (int i = 1; i < nBinsX; ++i) {
    if (std::abs(xaxis->GetBinWidth(i) / binSizeX - 1.) > 1e-9) throw BinSizeException();
  }
  for (int i = 1; i < nBinsY; ++i) {
    if (std::abs(yaxis->GetBinWidth(i) / binSizeY - 1.) > 1e-9) throw BinSizeException();
  }
  ...
  for (int j = -1; j < nBinsY - 1; ++j) {
    for (int i = -1; i < nBinsX - 1; ++i) {
      const double rhs[NCoeff] = {
          ...
          histcont(h, i, j), histcont(h, i + 1, j), histcont(h, i, j + 1), histcont(h, i + 1, j + 1),
          // df/dx in bin centres (finite difference approximation)
          dhistdx(h, i, j), dhistdx(h, i + 1, j), dhistdx(h, i, j + 1), dhistdx(h, i + 1, j + 1),
          // df/dy in bin centres (finite difference approximation)
          dhistdy(h, i, j), dhistdy(h, i + 1, j), dhistdy(h, i, j + 1), dhistdy(h, i + 1, j + 1),
          // d^2f/dxdy in bin centres (finite difference approximation)
          d2histdxdy(h, i, j), d2histdxdy(h, i + 1, j), d2histdxdy(h, i, j + 1), d2histdxdy(h, i + 1, j + 1)};
      ...
      coeff(1 + i, 1 + j, 5) = 9. * (rhs[0] - rhs[1] - rhs[2] + rhs[3]) + 6. * (rhs[4] - rhs[6] + rhs[8] - rhs[9]) +
                               4. * rhs[12] + 3. * (rhs[5] - rhs[7] + rhs[10] - rhs[11]) + 2. * (rhs[13] + rhs[14]) +
                               rhs[15];
      coeff(1 + i, 1 + j, 4) = 6. * (-rhs[0] + rhs[1] + rhs[2] - rhs[3]) + 4. * (-rhs[8] + rhs[9]) +
                               3. * (-rhs[4] - rhs[5] + rhs[6] + rhs[7]) +
                               2. * (-rhs[10] + rhs[11] - rhs[12] - rhs[13]) - rhs[14] - rhs[15];
      // a_03 ... a_33
      coeff(1 + i, 1 + j, 3) = 2. * (rhs[0] - rhs[2]) + rhs[8] + rhs[10];
      coeff(1 + i, 1 + j, 2) = 2. * (rhs[4] - rhs[6]) + rhs[12] + rhs[14];
      coeff(1 + i, 1 + j, 1) = 6. * (-rhs[0] + rhs[1] + rhs[2] - rhs[3]) + 4. * (-rhs[4] + rhs[6]) +
                               3. * (-rhs[8] + rhs[9] - rhs[10] + rhs[11]) +
                               2. * (-rhs[5] + rhs[7] - rhs[12] - rhs[14]) - rhs[13] - rhs[15];
      coeff(1 + i, 1 + j, 0) = 4. * (rhs[0] - rhs[1] - rhs[2] + rhs[3]) +
                               2. * (rhs[4] + rhs[5] - rhs[6] - rhs[7] + rhs[8] - rhs[9] + rhs[10] - rhs[11]) +
      ...
      double sum = 0.;
      for (int k = 0; k < NCoeff; ++k) sum += coeff(1 + i, 1 + j, k) / double((4 - (k % 4)) * (4 - (k / 4)));
...
template <class BASE>
RooBinned2DBicubicBase<BASE>* RooBinned2DBicubicBase<BASE>::clone(const char* newname) const {
...
template <class BASE>
Int_t RooBinned2DBicubicBase<BASE>::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& integVars,
...
template <class BASE>
Double_t RooBinned2DBicubicBase<BASE>::analyticalIntegral(Int_t code, const char* rangeName) const {
  switch (code) {
  case 1:
...
double RooBinned2DBicubicBase<BASE>::eval(double x, double y) const {
  ...
  // get low edge of bin
  const double xlo = double(nBinsX - binx) / double(nBinsX) * xmin + double(binx) / double(nBinsX) * xmax;
  const double ylo = double(nBinsY - biny) / double(nBinsY) * ymin + double(biny) / double(nBinsY) * ymax;
  ...
  // sum up
  double retVal = 0.;
  for (int k = 0; k < NCoeff; ++k) retVal += coeff(binx, biny, k) * hxton[k % 4] * hyton[k / 4];
...
double RooBinned2DBicubicBase<BASE>::evalX(double x1, double x2, double y) const {
  ...
  for (int binx = 0; binx < nBinsX; ++binx) {
    ...
    const double xhi = double(nBinsX - binx - 1) / double(nBinsX) * xmin + double(binx + 1) / double(nBinsX) * xmax;
    if (xhi < x1) continue;
    const double xlo = double(nBinsX - binx) / double(nBinsX) * xmin + double(binx) / double(nBinsX) * xmax;
    if (xlo > x2) break;
    ...
    const double hxton[4] = {0.25 * (b * b * b * b - a * a * a * a), (b * b * b - a * a * a) / 3.,
                             0.5 * (b * b - a * a), b - a};
    double lsum = 0.;
    for (int k = 0; k < NCoeff; ++k) lsum += coeff(binx, biny, k) * hxton[k % 4] * hyton[k / 4];
    sum += lsum;
  }
...
double RooBinned2DBicubicBase<BASE>::evalY(double x, double y1, double y2) const {
  ...
  for (int biny = 0; biny < nBinsY; ++biny) {
    // get low/high edge of bin
    const double yhi = double(nBinsY - biny - 1) / double(nBinsY) * ymin + double(biny + 1) / double(nBinsY) * ymax;
    if (yhi < y1) continue;
    const double ylo = double(nBinsY - biny) / double(nBinsY) * ymin + double(biny) / double(nBinsY) * ymax;
    if (ylo > y2) break;
    // work out integration range
    ...
    // integrated monomials
    const double hyton[4] = {0.25 * (b * b * b * b - a * a * a * a), (b * b * b - a * a * a) / 3.,
                             0.5 * (b * b - a * a), b - a};
    double lsum = 0.;
    for (int k = 0; k < NCoeff; ++k) lsum += coeff(binx, biny, k) * hxton[k % 4] * hyton[k / 4];
    sum += lsum;
...
template <class BASE>
double RooBinned2DBicubicBase<BASE>::evalXY(double x1, double x2, double y1, double y2) const {
  ...
  for (int biny = 0; biny < nBinsY; ++biny) {
    ...
    // integrated monomials
    const double hyton[4] = {0.25 * (by * by * by * by - ay * ay * ay * ay), (by * by * by - ay * ay * ay) / 3.,
                             0.5 * (by * by - ay * ay), by - ay};
    for (int binx = 0; binx < nBinsX; ++binx) {
      // get low/high edge of bin
      const double xhi = double(nBinsX - binx - 1) / double(nBinsX) * xmin + double(binx + 1) / double(nBinsX) * xmax;
      if (xhi < x1) continue;
      ...
      // integrated monomials
      const double hxton[4] = {0.25 * (bx * bx * bx * bx - ax * ax * ax * ax), (bx * bx * bx - ax * ax * ax) / 3.,
                               0.5 * (bx * bx - ax * ax), bx - ax};
      // integrate over bin
