    ...
    cmd("", ' ', "")
{
    ...
    externalScanFiles = {""};
    externalScanLabels = {"External"};
    externalScanOnly = false;
    filenamechange = "";
    grid = false;
    group = "GammaCombo";
    groupPos = "";
...
void OptParser::parseArguments(int argc, char* argv[])
{
    ...
    TCLAP::MultiArg<string> externalScanArg("", "externalscan",
    "Load external scan data from ROOT file for overlay plotting. "
    "Format: --externalscan path/to/external_scan1.root",
    false,  "string");
    TCLAP::SwitchArg externalScanOnlyArg("","externalscanonly","Suppress primary combiner (hack to plot external scans alone)", false);

    TCLAP::MultiArg<string> externalLabelArg("", "externallabel",
    "Label for external scan in legend",
    false, "string");
  ...
  TCLAP::MultiArg<int> plot2dclArg("","2dcl","Plot '2d' confidence level contours in 2d plots.\n"
    ...
    vector<string> extScan_tmp = externalScanArg.getValue();  ///< can't assign directly because of TString cast
    for ( int i = 0; i < extScan_tmp.size(); i++ ) externalScanFiles.push_back(extScan_tmp[i]);
    vector<string> extLabel_tmp = externalLabelArg.getValue();  ///< can't assign directly because of TString cast
