...
void PDF_Datasets::initConstraints(const TString& setName) {
  ...
  if (data && pdf)
    minNll = pdf->createNLL(*data, RooFit::Extended(kTRUE),
...
void PDF_Datasets::initData(const TString& name) {
  ...
  if (pdf && getWorkspace()->obj(constraintName))
    minNll = pdf->createNLL(*data, RooFit::Extended(kTRUE),
                            RooFit::ExternalConstraints(*getWorkspace()->set(constraintName)))
                 ->getVal();
...
void PDF_Datasets::initObservables(const TString& setName) {
  ...
  if (!isPdfInitialized()) {
    std::cerr << "FATAL in PDF_Datasets::initObservables -- first call PDF_Datasets::initPdf to init the PDF!"
              << std::endl;
    exit(EXIT_FAILURE);
  }
  obsName = setName;
...
void PDF_Datasets::initGlobalObservables(const TString& setName) {
  ...
  if (!wspc->set(globalObsName)) {
    std::cerr << "FATAL in PDF_Datasets::initGlobalObservables -- RooArgSet " << setName << " not found in workspace"
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
void PDF_Datasets::initParameters(const TString& setName) {
  ...
  if (!isPdfInitialized()) {
    std::cout << "FATAL in PDF_Datasets::initParameters -- first call PDF_Datasets::initPdf to init the PDF!"
              << std::endl;
    exit(EXIT_FAILURE);
  }
  parName = setName;
...
void PDF_Datasets::initParameters(const vector<TString>& parNames) {
  ...
  if (!isPdfInitialized()) {
    std::cout << "FATAL in PDF_Generic_Abs::initParameters -- first call PDF_Generic_Abs::initPdf to init the PDF!"
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
void PDF_Datasets::initMultipdfCat(const TString& name) {
  ...
  if (!(isPdfSet || isMultipdfSet)) {
    std::cout << "ERROR in PDF_Datasets::initMultipdfCat -- Need to call initPDF() before initMultipdfCat()"
              << std::endl;
    exit(EXIT_FAILURE);
  }
  if (isPdfSet && !isMultipdfSet) {
    std::cout << "ERROR in PDF_Datasets::initMultipdfCat -- Pdf already set, but it's not a MultiPdf -> contradiction!"
              << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  if (multipdfCat)
    isMultipdfCatSet = true;
  else {
    std::cout << "FATAL in PDF_Datasets::initMultipdfCat -- Category: " << multipdfCatName << " not found in workspace"
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
void PDF_Datasets::initPDF(const TString& name) {
  ...
  if (wspc->pdf(pdfName)->InheritsFrom("RooMultiPdf")) {
    ...
    std::cout << "intitalising no multipdf. isPdfSet=" << isPdfSet << " isMultipdfSet=" << isMultipdfSet << "."
              << std::endl;
  ...
  if (isMultipdfSet)
    std::cout << "INFO in PDF_Datasets::initPDF -- PDF initialized (MultiPdf)" << std::endl;
  else
    std::cout << "INFO in PDF_Datasets::initPDF -- PDF initialized" << std::endl;

  if (data && getWorkspace()->obj(constraintName))
    minNll = pdf->createNLL(*data, RooFit::Extended(kTRUE),
                            RooFit::ExternalConstraints(*getWorkspace()->set(constraintName)))
                 ->getVal();
  return;
...
void PDF_Datasets::initBkgPDF(const TString& name) {
  // std::cout << "PDF_Datasets::initBkgPDF() currently useless, as the Bkg pdf is taken as the full pdf with scanvar=0"
  // << std::endl;
  if (isBkgPdfSet) {
...
void PDF_Datasets::setVarRange(const TString& varName, const TString& rangeName, const double& rangeMin,
  ...
  if (!(rangeName == "free" || rangeName == "phys" || rangeName == "scan" || rangeName == "bboos" ||
        rangeName == "force")) {
...
RooFitResult* PDF_Datasets::fit(RooAbsData* dataToFit) {
  ...
  // unfortunately Minuit2 does not initialize the status of the roofitresult, if all parameters are constant. Therefore
...
RooFitResult* PDF_Datasets::fitBkg(RooAbsData* dataToFit, TString signalvar) {
  ...
  //     // exit(EXIT_FAILURE);
  // }
  // std::cout << "WARNING in PDF_Datasets::fitBkg -- Fitting bkg model as sig+bkg model with " << signalvar << " to
  // zero!" << std::endl;
  if (pdfBkg) {
    ...
    RooMsgService::instance().setGlobalKillBelow(ERROR);
    RooMsgService::instance().setSilentMode(kTRUE);
    // unfortunately Minuit2 does not initialize the status of the roofitresult, if all parameters are constant.
    // Therefore need to stay with standard Minuit fitting. RooFitResult* result  = pdfBkg->fitTo( *dataToFit,
...
void PDF_Datasets::generateToys(int SeedShift) {
  ...
  initializeRandomGenerator(SeedShift);
  // RooDataSet* toys = this->pdf->generate(*observables, RooFit::NumEvents(wspc->data(dataName)->numEntries()),
  // RooFit::Extended(kTRUE));
  //
  RooAbsData* toys;
  if (isMultipdfSet) {
    toys = multipdf->getPdf(bestIndexScan)
               ->generate(*observables, wspc->data(dataName)->numEntries(), false, true, "", false, true);
  } else {
...
void PDF_Datasets::generateBkgToys(int SeedShift, TString signalvar) {
  ...
  //     // exit(EXIT_FAILURE);
  // }
  // std::cout << "WARNING in PDF_Datasets::generateBkgToys -- Fitting bkg model as sig+bkg model with " << signalvar <<
  // " to zero!" << std::endl;
  RooAbsData* toys;
  if (isBkgMultipdfSet) {
    toys = multipdfBkg->getPdf(bestIndexBkg)
               ->generate(*observables, wspc->data(dataName)->numEntries(), false, true, "", false, true);
    ...
    getWorkspace()->var(signalvar)->setVal(0.0);
    getWorkspace()->var(signalvar)->setConstant(true);
    // RooDataSet* toys = pdfBkg->generate(*observables, RooFit::NumEvents(wspc->data(dataName)->numEntries()),
    // RooFit::Extended(kTRUE)); RooDataSet* toys = pdf->generate(*observables,
    // RooFit::NumEvents(wspc->data(dataName)->numEntries()), RooFit::Extended(kTRUE));
    if (isMultipdfSet) {
      toys = multipdf->getPdf(bestIndexBkg)
                 ->generate(*observables, wspc->data(dataName)->numEntries(), false, true, "", false, true);
    } else {
      toys = pdf->generate(*observables, wspc->data(dataName)->numEntries(), false, true, "", false, true);
    }
...
void PDF_Datasets::unblind(TString var, TString unblindRegs) {
  ...
  if (!wspc->var(var)) {
    std::cerr << "ERROR::PDF_Datasets::unblind(): the variable " << var << " is not present in the workspace."
              << std::endl;
    if (observables) {
