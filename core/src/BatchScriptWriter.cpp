...
void BatchScriptWriter::writeScripts(const OptParser* arg, const std::vector<std::unique_ptr<Combiner>>& cmb) {
  ...
  for (int i = 0; i < arg->combid.size(); i++) {
    ...
    TString outf_dir = TString(filesystem::current_path()) + "/root/" + dirname;
...
void BatchScriptWriter::writeCondorScript(TString fname, const OptParser* arg) {
  ...
  const auto cwd = filesystem::current_path().c_str();
