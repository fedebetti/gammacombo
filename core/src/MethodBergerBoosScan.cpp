...
/// be the default.
///
MethodBergerBoosScan::MethodBergerBoosScan(MethodProbScan* s, TString d) : MethodPluginScan(s) {
...
void MethodBergerBoosScan::drawBBPoints(TString varX, TString varY, int runMin, int runMax, bool save) {
  ...
  for (int entry = 0; entry < tree->GetEntries(); entry++) {
    ...
    if (hBBPoints->GetBinContent(nBinX, nBinY) <= nBBPoints && hBBPoints->GetBinContent(nBinX, nBinY) != 0 &&
        hBBPoints->GetBinContent(nBinX, nBinY) != (tree->GetLeaf("BergerBoos_id")->GetValue() + 1)) {
      ...
      hBBPoints->SetBinContent(
          nBinX, nBinY, (tree->GetLeaf("BergerBoos_id")->GetValue() + 1 + hBBPoints->GetBinContent(nBinX, nBinY)));
    } else {
...
void MethodBergerBoosScan::readScan1dTrees(int runMin, int runMax) {
  ...
  for (int i = runMin; i <= runMax; i++) {
    ...
    //     std::cout << "MethodBergerBoosScan::readScan1dTrees() : " << ((float)i/(float)(runMax-runMin))*100 << "\% of
    //     files read." << std::endl;
    //   }
    // }
    TString file = Form(fileNameBase + "%i.root", i);
  ...
  for (Long64_t i = 0; i < nentries; i++) {
    if ((i + 1) % (int)(nentries / 10) == 0) {
      std::cout << "MethodBergerBoosScan::readScan1dTrees() : " << Form("%2.0f", ((float)i / (float)nentries) * 100)
    ...
    if (!(fabs(t.chi2minToy) < 500 && fabs(t.chi2minGlobalToy) < 500 && t.statusFree == 0. && t.statusScan == 0.
      ...
      if (arg->debug) {
        ...
        if (fabs(t.chi2minToy) >= 500) { std::cout << "|t.chi2minToy| >= 500 " << std::endl; }
        if (fabs(t.chi2minGlobalToy) >= 500) { std::cout << "|t.chi2minGlobalToy| >= 500 " << std::endl; }
        if (fabs(t.chi2minGlobal - chi2minGlobal) >= 0.1) {
          std::cout << "|t.chi2minGlobal-chi2minGlobal| >= 0.1 " << std::endl;
        }
        if (t.statusFree == 1) { std::cout << "t.statusFree == 1 " << std::endl; }
        if (t.statusScan == 1) { std::cout << "t.statusScan == 1 " << std::endl; }
      }
      nfailed++;
      continue;
    }
    ...
    if (inPhysicalRegion) { h_all->Fill(t.scanpoint, t.BergerBoos_id); }
    ...
    if (!inPhysicalRegion) { h_background->Fill(t.scanpoint, t.BergerBoos_id); }
  }
