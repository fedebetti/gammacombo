...
void OneMinusClPlot::Draw()
{
    bool plotSimple = false;//arg->debug; ///< set to true to use a simpler plot function
        ...
        cout << "DEBUG: Number of scanners: " << scanners.size() << endl;
for ( int i = 0; i < scanners.size(); i++ ) {
    cout << "DEBUG: Scanner " << i << " - " << scanners[i]->getTitle()<< " Fill: " << scanners[i]->getFillStyle()<< " Color: " << scanners[i]->getFillColor() << endl;
}
