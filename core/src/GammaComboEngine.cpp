...
ExternalScanWrapper* GammaComboEngine::createExternalScanner(TString filename, TString label) {
    ...
    if (!extFile || extFile->IsZombie()) {
        cout << "ERROR: Cannot open external scan file: " << filename << endl;
        return nullptr;
    }
    ...
    if (!hCL_obj) {
        cout << "ERROR: No hCL histogram found in file" << endl;
        extFile->Close();
        return nullptr;
    }
    ...
    if (is2D) {
                cout << "Loading 2D external scan..." << endl;
	...
	if (hCL2d) {
        ...
        delete extFile;
	        ...
	        if (!hCL2d) {
            cout << "ERROR: Failed to load or clone hCL histogram" << endl;
            return nullptr;
        }
        extScanner = new ExternalScanWrapper(hCL2d, hCLs2d, "external_" + label, label, arg);
            ...
            if (xName.Contains("gamma") || xName.Contains("delta") || xName.Contains("phi") || 
                xName.Contains("theta") || xName.Contains("alpha") || xName.Contains("beta")) {
                extScanner->getScanVar1()->setUnit("Rad");
            }
            if (yName.Contains("gamma") || yName.Contains("delta") || yName.Contains("phi") || 
                yName.Contains("theta") || yName.Contains("alpha") || yName.Contains("beta")) {
                extScanner->getScanVar2()->setUnit("Rad");
            }
        
    } else {
        cout << "Loading 1D external scan..." << endl;
        ...
        if (!hCL1d) {
            cout << "ERROR: hCL exists but is not TH1F or TH2F" << endl;
            extFile->Close();
            return nullptr;
        }
...
void GammaComboEngine::make1dProbPlot(MethodProbScan *scanner, int cId)
{
	...
	// Add external scanner if requested and only do it after last internal scanner
	    ...
	    if( files_len!=labels_len) {cout<< "WARNING: More external files provided than labels"<< endl;}
	    for (int i=1; i<files_len;i++)
	    {	
                ExternalScanWrapper* extScanner = (i<labels_len) ? createExternalScanner(arg->externalScanFiles[i], arg->externalScanLabels[i]):createExternalScanner(arg->externalScanFiles[i], TString("External "+std::to_string(i)));

	        if(arg->debug)extScanner->getHCL2d();
                if (extScanner) 
		{

                    ((OneMinusClPlot*)plot)->addScanner(extScanner, 0);
                }
	    }
        }
	...
	// Add external scanner if requested, and only do it after last scanner
	    ...
	    {	
	        ...
	        if(arg->debug)extScanner->getHCL2d();
                if (extScanner) 
		{
                    ((OneMinusClPlot2d*)plot)->addScanner(extScanner, 0);
                }
	    }
        }
...
{
    cout << "DEBUG: scan() method called" << endl;
    cout << "DEBUG: runOnDataSet = " << (runOnDataSet ? "true" : "false") << endl;
    cout << "DEBUG: arg->var.size() = " << arg->var.size() << endl;
    // if we're running with the dataset option then we go off and do that somewhere else
    if ( runOnDataSet )
    {
	cout << "DEBUG: Taking dataset path" << endl;
        scanDataSet();
        return;
    }

    cout << "DEBUG: Taking combiner path, number of combiners: " << arg->combid.size() << endl;
    ...
    for ( int i=0; i<arg->combid.size(); i++ )
    {
        ...
        if ( arg->compare ) comparisonScanners.push_back(scannerProb);
            ...
            {
                if ( arg->isAction("plot") ){
			     cout << "DEBUG: Loading scanner for 2D plot" << endl;
                    scannerProb->loadScanner(m_fnamebuilder->getFileNameScanner(scannerProb));
                }
                else{
			    cout << "DEBUG: Running 2D prob scan" << endl;
                    make2dProbScan(scannerProb, i);
                }
		 cout << "DEBUG: About to call make2dProbPlot" << endl;
