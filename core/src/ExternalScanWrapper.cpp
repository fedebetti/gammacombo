      ...
      is2D(false) 
{
    ...
    scanVar1Dummy = new RooRealVar(
        xName.IsNull() ? "x" : xName, 
        xTitle.IsNull() ? "x" : xTitle, 
      ...
      is2D(true) 
{
    ...
    scanVar1Dummy = new RooRealVar(xName.IsNull() ? "x" : xName,
                                    xTitle.IsNull() ? "x" : xTitle,
                                    xCenter, xMin, xMax);
    scanVar2Dummy = new RooRealVar(yName.IsNull() ? "y" : yName,
                                    yTitle.IsNull() ? "y" : yTitle,
                                    yCenter, yMin, yMax);
}
