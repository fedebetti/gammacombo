    ...
    for (int j = 0; j < arg->physRanges[0].size(); j++) {
      if (w->var(arg->physRanges[0][j].name) &&
          w->set(pdf->getParName())->find(arg->physRanges[0][j].name)) {  // if somebody wants to modify a constant
                                                                          // parameter make sure the parameter doesn't
                                                                          // accidentally become floating...
...
TChain* MethodDatasetsPluginScan::readFiles(int runMin, int runMax, int& nFilesRead, int& nFilesMissing,
  ...
  TString fileNameBase = (fileNameBaseIn.EqualTo("default"))
                             ? dirname + "/scan1dDatasetsPlugin_" + this->pdf->getName() + "_" + scanVar1 + "_run"
                             : fileNameBaseIn;
...
///
/// This is the Plugin version of the method, there is also a version of the method for the prob scan, with _prob
/// suffix. \todo: Like for the normal gammacombo stuff, use a seperate class for the PROB scan! This is
/// MethodDatasetsPLUGINScan.cpp, after all
/////////////
void MethodDatasetsPluginScan::readScan1dTrees(int runMin, int runMax, TString fileNameBaseIn) {
  ...
  double halfBinWidth = (t.getScanpointMax() - t.getScanpointMin()) / ((float)t.getScanpointN()) / 2;  //-1.)/2;
  ...
  TH1F* h_sig_bkgtoys = new TH1F("h_sig_bkgtoys", "signal distribution for bkg toys", 50,
                                 -5 * (w->var(scanVar1)->getError()), 5 * (w->var(scanVar1)->getError()));
  ...
  std::cout << "MethodDatasetsPluginScan::readScan1dTrees() : average number of toys per scanpoint: "
            << (double)nentries / nPoints1d << std::endl;
  Long64_t nfailed = 0;
  ...
  for (Long64_t i = 0; i < nentries; i++) {
    ...
    // bool BadBkgFit          = (!(t.statusFreeBkg == 0 && t.statusBkgBkg == 0) && (t.covQualFreeBkg == 3 &&
    // t.covQualBkgBkg == 3))||(std::isnan(t.chi2minBkgBkgToy - t.chi2minGlobalBkgToy)||(t.chi2minBkgBkgToy -
    // t.chi2minGlobalBkgToy<0)||(t.statusBkgBkg!=0)||(t.statusFreeBkg!=0));
    bool BadBkgFit =
        (!(t.statusFreeBkg == 0 && t.statusScanBkg == 0) && (t.covQualFreeBkg == 3 && t.covQualScanBkg == 3)) ||
        (std::isnan(t.chi2minBkgToy - t.chi2minGlobalBkgToy) ||
         (abs(t.chi2minBkgToy) > 1e27 || abs(t.chi2minGlobalBkgToy) > 1e27));
    ...
    if (BadBkgFit) {
      if (arg->debug) {
        ...
        if (t.covQualFreeBkg != 3)
          std::cout << "imperfect free fit covariance matrix (status " << t.covQualFreeBkg << "), ";
        if (t.covQualScanBkg != 3)
          std::cout << "imperfect scan fit covariance matrix (status " << t.covQualScanBkg << "), ";
        if (std::isnan(t.chi2minBkgToy - t.chi2minGlobalBkgToy) ||
            (abs(t.chi2minBkgToy) > 1e27 || abs(t.chi2minGlobalBkgToy) > 1e27))
          std::cout << "and problem with likelihood computations.";
        std::cout << std::endl;
      }
    ...
    if (arg->teststatistic == 1) {                                               // use one-sided test statistic
      teststat_measured = bestfitpoint <= t.scanpoint ? teststat_measured : 0.;  // if mu < muhat then q_mu = 0 //best
                                                                                 // fit point defined in constructor
      sb_teststat_toy = t.scanbest <= t.scanpoint ? sb_teststat_toy : 0.;        // if mu < muhat then q_mu = 0
      b_teststat_toy = t.scanbestBkg <= t.scanpoint ? b_teststat_toy : 0.;       // if mu < muhat then q_mu = 0
    }
    ...
    if (!BadBkgFit && (b_teststat_toy) >= (teststat_measured)) { h_better_clb->Fill(t.scanpoint); }
    ...
    if (valid) {  // inPhysicalRegion )
      ...
      // h_probPValues->SetBinContent(h_probPValues->FindBin(t.scanpoint), this->getPValueTTestStatistic(t.chi2min -
      // this->chi2minGlobal)); //t.chi2minGlobal));
      h_probPValues->SetBinContent(h_probPValues->FindBin(t.scanpoint),
                                   this->getPValueTTestStatistic(teststat_measured));  // t.chi2minGlobal));
    ...
    // ToDo: is that really sensible? Currently only used for control plots and background is estimated in a dedicated
    // way
    if (valid && !inPhysicalRegion) { h_background->Fill(t.scanpoint); }

    if (n0tot % 1500 == 0 && n0all != 0) {
      // std::cout << "better: " << n0better << " all: " << n0all << " p: " << (double)n0better/n0all << std::endl
      // << std::endl;
      h_pVals->Fill((double)n0better / n0all);
      n0tot = 0;
      n0better = 0;
      n0all = 0;
    }
  ...
  std::cout << "MethodDatasetsPluginScan::readScan1dTrees() : read an average of "
            << ((double)nentries - (double)nfailed) / nPoints1d << " toys per scan point." << std::endl;
  ...
  std::cout << "MethodDatasetsPluginScan::readScan1dTrees() : fraction of unphysical (negative test stat) toys: "
            << h_background->GetEntries() / nentries * 100. << "%." << std::endl;
  ...
  if (nwrongrun > 0) {
    std::cout
        << "\nMethodDatasetsPluginScan::readScan1dTrees() : WARNING : Read toys that differ in global chi2min (wrong "
           "run) : "
        << (double)nwrongrun / (nentries - nfailed) * 100. << "%.\n"
        << std::endl;
  }
  ...
  for (int i = 1; i <= h_better->GetNbinsX(); i++) {
    ...
    // double mederr_asymptotic_val = sqrt( probs[2]*( 1.-probs[2] ) / ( cls_vals.size()*bkg_pvals_cls->GetBinContent(
    ...
    // (quantiles_cls_low_high_med[1]-quantiles_cls_low_high_med[0])/2.);

    // mederr_poisson_cls->SetBinContent(i,sqrt((1.-TMath::Min( quantiles_cls[2] , 1.))*TMath::Min( quantiles_cls[2]
    // , 1.)/sampledBValues[i].size())); mederr_poisson_clsb_clb->SetBinContent(i,sqrt(
    // (quantiles_clsb[2]*(1.-quantiles_clsb[2])/sampledSchi2Values[i].size()) +
    // (quantiles_clb[2]*(1.-quantiles_clb[2])/sampledBValues[i].size())));
    // //...
  ...
  if (arg->controlplot) {
    ...
    // ToDo: the tayloring of those control plots does not suit the datasets case as there is no normalisation to
    // chi2minGlobal ControlPlots cp(&t); if ( arg->plotid==0 || arg->plotid==1 ) cp.ctrlPlotMore(profileLH); if (
    // arg->plotid==0 || arg->plotid==2 ) cp.ctrlPlotChi2(); if ( arg->plotid==0 || arg->plotid==3 )
    // cp.ctrlPlotNuisances(); if ( arg->plotid==0 || arg->plotid==4 ) cp.ctrlPlotObservables(); if ( arg->plotid==0 ||
  ...
  if (arg->debug || arg->controlplot) {
    ...
    leg->SetHeader(("   " + std::to_string(int((double)nentries / nPoints1d)) + " toys").c_str());
    ...
    leg_neg->SetHeader(("   " + std::to_string(int((double)nentries / nPoints1d)) + " toys").c_str());
...
int MethodDatasetsPluginScan::scan1d(int nRun) {
  ...
  std::vector<double> chi2minGlobalBkgToysStore;  // Global fit to bkg-only toys
  std::vector<double> chi2minBkgBkgToysStore;     // Bkg fit to bkg-only toys
  std::vector<double> scanbestBkgToysStore;       // best fit point of gloabl fit to bkg-only toys
  std::vector<double> scanbestBkgBkgToysStore;  // best fit point of bkg fit to bkg-only toys (usually zero because bkg
  ...
  for (int j = 0; j < nActualToys; j++) {
    ...
    // if(pdf->getBkgPdf())
    {
      Utils::setParameters(w, dataBkgFitResult);  // set parameters to bkg fit so the generation always starts at the
                                                  // same value
      // pdf->printParameters();
      pdf->generateBkgToys(0, arg->var[0]);
      pdf->generateBkgToysGlobalObservables(0, j);
      RooAbsData* bkgOnlyToy = pdf->getBkgToyObservables();
      cls_bkgOnlyToys.push_back((RooAbsData*)bkgOnlyToy->Clone());  // clone required because of deleteToys() call at
                                                                    // end of loop
      ...
      Utils::setParameters(w, rb);  // set parameters to fitresult of best fit before making refitting decision,
      ...
      // if the pdf does not depend on the signal parameter, set best fit value of signa l parameter for the bkg fit to
      // 0
      else
        scanbestBkgToysStore.push_back(0.0);
        ...
        for (int j = 0; j < arg->physRanges[0].size(); j++) {
          if (w->var(arg->physRanges[0][j].name) &&
              w->set(pdf->getParName())->find(arg->physRanges[0][j].name)) {  // if somebody wants to modify a constant
                                                                              ...
                                                                              // doesn't accidentally become floating...
            w->var(arg->physRanges[0][j].name)->setConstant(false);
          }
        }
      }
      ...
      if (rb->floatParsFinal().find(scanVar1)) {
        std::cout << "found signal parameter in bkg fit with value "
                  << ((RooRealVar*)w->set(pdf->getParName())->find(scanVar1))->getVal() << std::endl;
      ...
      // if the pdf does not depend on the signal parameter, set best fit value of signal parameter for the bkg fit to 0
        ...
        scanbestBkgBkgToysStore.push_back(0.0);
  ...
  // start scan
  std::cout << "MethodDatasetsPluginScan::scan1d_plugin() : starting ... with " << nPoints1d << " scanpoints..."
            << std::endl;
  ...
  for (int i = 0; i < nPoints1d; i++) {
    ...
    double scanpoint = parameterToScan_min + (parameterToScan_max - parameterToScan_min) * (double)i / (nPoints1d - 1);
    ...
    // Load the parameter values (nuisance parameters and parameter of interest) from the fit to data with fixed
    ...
    for (int j = 0; j < nActualToys; j++) {
      ...
      // For toy generation, set all parameters (nuisance parameters and parameter of interest) to the values from the
      // constrained fit to data with fixed parameter of interest. This is called the PLUGIN method.
      this->setParevolPointByIndex(i);
      ...
      this->pdf->generateToysGlobalObservables();  // this is generating the toy global observables and saves globalObs
                                                   // in snapshot
      ...
      Utils::setParameters(w, r1);  //  set parameters to fitresult of best fit before making refitting decision,
                                    //  necessary if using multipdf
      // implement physical range a la Feldman Cousins
      bool refit_necessary = false;
...
void MethodDatasetsPluginScan::drawDebugPlots(int runMin, int runMax, TString fileNameBaseIn) {
  ...
  can2->cd();
  chain->Draw("chi2minToy-chi2minGlobalToy",
              "!(" + cut + "&&" + isphysical + ") && abs(chi2minToy-chi2minGlobalToy)<1e2", "norm");
  can3->cd();
...
void MethodDatasetsPluginScan::performBootstrapTest(int nSamples, const TString& ext) {
  ...
  this->readFiles(arg->jmin[0], arg->jmax[0], nFilesRead, nFilesMissing, arg->jobdir);
...
RooSlimFitResult* MethodDatasetsPluginScan::getParevolPoint(double scanpoint) {
