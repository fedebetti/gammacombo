...
class SharedArrayImp : public TObject {
 public:
  ...
  /// read-only access
  const TYPE& operator[](unsigned idx) const { return arr[idx]; }
  /// read-write access
  TYPE& operator[](unsigned idx) { return arr[idx]; }
...
class SharedArray : public TObject {
 ...
 public:
  /// constructor
  SharedArray(unsigned sz) : pimpl(sz ? (new SharedArrayImp<TYPE>(sz)) : 0) {}
  /// copy constructor
  SharedArray(const SharedArray<TYPE>& other) : TObject(other), pimpl(other.pimpl->acquire()) {}
  /// clone method
  ...
  const RWProxy operator[](unsigned idx) const { return RWProxy(const_cast<SharedArray<TYPE>*>(this), idx); }
  /// read-write access to array elements
  RWProxy operator[](unsigned idx) { return RWProxy(this, idx); }
 ...
 public:
  ...
  class RWProxy {
   public:
    /// constructor
    RWProxy(SharedArray* par, unsigned i) : parent(par), idx(i) {}

    /// rvalue use
    operator TYPE() const { return parent->pimpl->operator[](idx); }
    ...
    RWProxy& operator=(const TYPE d) {
      ...
      if (1 < parent->pimpl->refCount()) {
        ...
        // trigger deep copy
        if (d == parent->pimpl->operator[](idx)) return *this;
        parent->triggerCOW();
      }
      // perform assignment
    ...
    RWProxy& operator=(const RWProxy& other) {
      ...
      if (other.idx == idx && other.parent->pimpl == parent->pimpl) return *this;
      return (*this = TYPE(other));
    }

    /// addition and assignment
    RWProxy& operator+=(const TYPE d) { return *this = TYPE(*this) + d; }
    /// addition and assignment
    RWProxy& operator+=(const RWProxy& other) { return *this += TYPE(other); }
    ...
    /// equality
    bool operator==(const TYPE d) const { return TYPE(*this) == d; }
    /// equality
    bool operator==(const RWProxy& other) const { return *this == TYPE(other); }

    /// non-equality
    bool operator!=(const TYPE d) const { return !(*this == d); }
    /// non-equality
    bool operator!=(const RWProxy& other) const { return !(*this == other); }
