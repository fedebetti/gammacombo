...
int main(int argc, char* argv[]) {
  ...
  // This allows the calculation of the limit on a dataset which includes the systematic uncertainty due to model
  // choice, using the discrete profiling method https://arxiv.org/abs/1408.6865
  //
  ...
  //    - The only difference is the need to call pdf->initMultipdfCat to initialise the category which indexes the
  //    multipdf
  //
  // It is run in the same way as Lb2pktaul_highBDTCut_dataset.cpp:
  // 1. Profile likelihood scan - For a multipdf, this will not return the correct limit, but is necessary setup for the
  // plugin scan bin/Lb2pktaul_highBDTCut_dataset_multipdf --var branchingRatio --npoints 20 --scanrange 0.:7.e-7
  // --teststat 1
  // 2. Plugin scan
  // bin/Lb2pktaul_highBDTCut_dataset_multipdf -a pluginbatch --var branchingRatio --npoints 20 --npointstoy 20
  // --scanrange 0.:7.e-7
  // --ntoys 50 --teststat 1 bin/Lb2pktaul_highBDTCut_dataset_multipdf -a plugin --var branchingRatio --npoints 20
  // --npointstoy 20
  ...
  // is selected for comparison because this is the pdf which achieves the global minimum in this case)
