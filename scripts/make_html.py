...
parser.add_option(
    "-p", "--plotsPerLine", type="int", default=-1, help="Plots to display per line"
)
parser.add_option(
    "-c",
    "--colorScheme",
    default="maroon",
    help="html color (as string) for borders etc.",
)
parser.add_option(
    "-u", "--upload", default=None, help="Upload location on afs web server"
)
parser.add_option(
    "-l",
    "--lxplus",
    default=False,
    action="store_true",
    help="If already running on lxplus",
)
parser.add_option(
    "-T",
    "--date",
    default=None,
    help="Filter only files modified after this date - format dd/mm/yyyy. Default=%default",
)
parser.add_option(
    "-r", "--regex", default=None, help="Each plot name must match this regex"
)
...
parser.add_option(
    ...
    "--newLoc",
    default=None,
    help="Copy plots and html file into a seperate location",
)
parser.add_option(
    "-s",
    "--sort",
    default=None,
    help="Sort files by name (ascending=an,descending=dn) or time modified (ascending=at, descending=dt)",
)
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Convert pdfs to high resolution pngs (will overwrite existing pngs)",
)
...
def gatherFiles(path):
    ...
    for root, dirs, files in os.walk(path):
        ...
        for fil in files:
            ...
            if opts.date:
                d = datetime.datetime(
                    int(opts.date.split("/")[2]),
                    int(opts.date.split("/")[1]),
                    int(opts.date.split("/")[0]),
                )
...
def writeHtml(location, title, links, plots, isHome=False):
    ...
    html.write(
        '<script language="Javascript"> document.write("Last modified: " + document.lastModified + " (UTC)"); </script> <br>\n'
    )
    html.write("<br>\n")
    ...
    # contents
    html.write(
        '<div onClick="openClose(\'contents\')" style="cursor:hand; cursor:pointer"><b><u><font color="blue"> Contents: </font></u></b> (click to expand) </div> \n'
    )
    ...
    if opts.zip:
        ...
        if opts.newLoc:
            html.write(
                "<div><b>Download all: <a href=%s.zip>%s.zip</a></b></div>\n"
                % (opts.zip, opts.zip)
            )
        else:
            html.write(
    ...
    for i, png in enumerate(keys):
        ...
        if "pdf" in list(plots.keys()):
            ...
            if opts.convert:
                cmd = "sips -s format png plots/%s --out plots/%s > /dev/null" % (
                    pdf_path,
                    png_path,
                )
                os.system(cmd)
        ...
        html.write(
            '<div style="display:inline-block;border-style:groove;border-width:5px;color:%s;width:%s;word-break:break-all;word-wrap:break-all;">\n'
        ...
        if opts.plotsPerLine > 0 and (i + 1) % opts.plotsPerLine == 0:
            html.write("<br>\n")
