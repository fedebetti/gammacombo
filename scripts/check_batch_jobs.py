...
parser = OptionParser()
parser.add_option(
    "-d", "--dir", default="sub", help="Directory to check in. Default=%default"
)
parser.add_option(
    "-s",
    "--synch",
    default=False,
    action="store_true",
    help="Synch the output root files to the root directory. Default=%default",
)
parser.add_option(
    "-S",
    "--synchdir",
    default=None,
    help="Synch to this directory (if different from directory with jobs scripts e.g. if files are stored on eos. Default=%default",
)
parser.add_option(
    "-r", "--regex", default=None, help="Filter folders by this regex. Default=%default"
)
parser.add_option(
    "-t",
    "--date",
    default=None,
    help="Filter only folders modified after this date - format dd/mm/yyyy. Default=%default",
)
parser.add_option(
    "-R",
    "--resubmit",
    default=None,
    help="Resubmit jobs (pass Queued, Failed, Running, All). Completed jobs don't get resubmitted. Default=%default",
)
parser.add_option(
    "-q", "--queue", default=None, help="Queue to resubmit jobs to. Default=%default"
)
parser.add_option(
    "-n",
    "--skipBackUp",
    default=False,
    action="store_true",
    help="Dont backup old files. Default=%default",
)
parser.add_option(
    "-c",
    "--clearLinks",
    default=False,
    action="store_true",
    help="Clear old files / links out the way",
)
    ...
    if root not in job_dirs:
        ...
        if include:
            ...
            if not os.path.exists(subpath):
                raise RuntimeError(
                    f"Couldn't find the submission file {subpath} for job {root}"
                )
...
for job_dir, ctime in job_dirs:
    ...
    for root, dirs, files in os.walk(job_dir):
        ...
        print(job_dir)
        if len(waiting) > 0:
            print("\tQueued:  ", len(waiting), "/", len(scripts))
        if len(fail) > 0:
            print("\tFailed:  ", len(fail), "/", len(scripts))
        if len(run) > 0:
            print("\tRunning: ", len(run), "/", len(scripts))
        if len(done) > 0:
            print("\tComplete:", len(done), "/", len(scripts))
...
# print total
print("TOTAL")
if len(total_waiting) > 0:
    print("\tQueued:  ", len(total_waiting), "/", len(total_scripts))
if len(total_fail) > 0:
    print("\tFailed:  ", len(total_fail), "/", len(total_scripts))
if len(total_run) > 0:
    print("\tRunning: ", len(total_run), "/", len(total_scripts))
if len(total_done) > 0:
