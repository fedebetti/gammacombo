...
parser.add_option(
    ...
    "--git_extension",
    default="status",
    help="Extension to git command to execute",
)
parser.add_option(
    "-c",
    "--useCombinersFile",
    default=False,
    action="store_true",
    help="Use cmake/combiners.cmake file to get paths",
)
...
def printLine(text=""):
    ...
    to_print = ""
    for i in range(front_dashes):
        to_print += "-"
    to_print += text
    for i in range(back_dashes):
        to_print += "-"
    print(to_print)
