raise RuntimeError(
    "gc_mpl.py is depreciated (and marked for removal) and is no longer supported use gc_mpl_tools.py instead"
)
...
parser.add_argument(
    "-s", "--scan", default=[], action="append", help="Scanner to pick up"
)
...
def read2dscan(h, bf, minnll):
    ...
    xedges = np.array(
        [h.GetXaxis().GetBinLowEdge(b) for b in range(1, h.GetNbinsX() + 2)]
    )
    yedges = np.array(
        [h.GetYaxis().GetBinLowEdge(b) for b in range(1, h.GetNbinsX() + 2)]
    )
    xcenters = np.array(
...
def plot1d(fname, scans, xvar, fill=True, interactive=False):
    ...
    if fill:
        for scan in scans:
            ax.fill_between(scan.x, 0.0, scan.y, **scan.fopts)
...
def plot2d(fname, scans, xvar, yvar, xlim=None, ylim=None, ncontours=2, cl2d=False):
    ...
    for i, scan in enumerate(scans):
        ...
        if (
            "colors" in scan.fopts.keys()
            and "colors" in scan.lopts.keys()
            and ("label" in scan.fopts.keys() or "label" in scan.lopts.keys())
        ):
