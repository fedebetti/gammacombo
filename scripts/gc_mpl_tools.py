...
def read_external_scan(rootfile, scanvar1=None, scanvar2=None):
  ...
  if is2D:
      # 2D case
      xedges = np.array([hCL.GetXaxis().GetBinLowEdge(b) for b in range(1, hCL.GetNbinsX()+2)])
      yedges = np.array([hCL.GetYaxis().GetBinLowEdge(b) for b in range(1, hCL.GetNbinsY()+2)])
      xcenters = np.array([hCL.GetXaxis().GetBinCenter(b) for b in range(1, hCL.GetNbinsX()+1)])
      ycenters = np.array([hCL.GetYaxis().GetBinCenter(b) for b in range(1, hCL.GetNbinsY()+1)])

      # Get bin contents (already as p-values)
      entries = np.array([hCL.GetBinContent(xbin+1, ybin+1)
                         for xbin, ybin in itertools.product(range(hCL.GetNbinsX()),
                                                             range(hCL.GetNbinsY()))])
