...
parser.add_argument(
    "-f", "--list", default=None, help="Read files as a list from another file"
)
parser.add_argument("-p", "--par", default=None, help="Give parameter name")
parser.add_argument(
    "-b", "--bestfit", default=None, help="Add a best fit point val:file"
)
parser.add_argument(
    "-l", "--limit", default=None, help="Limit for observed parameter values"
)
...
for fil in files:
    ...
    for cl, exp in zip(["68", "95", "99"], ["68.27", "95.45", "99.73"]):
        ...
        print_rows.append(
            [exp, fr_prob * 100, er_prob * 100, fr_plug * 100, er_plug * 100]
        )

        print(
            f"${exp:5s}$ & $({fr_prob * 100:5.2f} \\pm {er_prob * 100:4.2f})$ & ${rs_prob * 100: 4.2f}$ & $({fr_plug * 100:5.2f} \\pm {er_plug * 100:4.2f})$ & ${rs_plug * 100: 4.2f}$ \\\\"
        )
    print("\\hline\\hline")
    tf.Close()
...
for nsig in range(1, 4):
    ...
    if args.bffil:
        ...
        ax[nsig - 1].errorbar(
            [args.bfval], [y_prob_pltv[0]], yerr=[y_prob_plte[0]], fmt="go"
        )
        ax[nsig - 1].errorbar(
            [args.bfval], [y_plug_pltv[0]], yerr=[y_plug_plte[0]], fmt="gx", label=label
        )
...
print(
    tabulate(
        print_rows, headers=["Expected", "Prob", "Err", "Plugin", "Err"], floatfmt=".1f"
    )
)
if args.interactive:
    plt.show()
