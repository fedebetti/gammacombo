...
os.chdir("tutorial")
if not os.path.exists("ci_logs"):
    os.mkdir("ci_logs")

global htmlf
htmlf = open("ci_logs/index.html", "w")
htmlf.write(
    '<font size="5"><u> GammaCombo Continous Integration Test </u></font><br>\n'
)
htmlf.write(
    '<script language="Javascript"> document.write("CI checks performed on " + document.lastModified +" (UTC)"); </script> <br>\n'
)
...
def check_comb_stdout(logf):
    ...
    for line in f.readlines():
        ...
        if line.startswith("a_gaus = [") and "Prob" in line:
            if "0.68CL" in line:
                assert "1.20 -   0.46 +   0.46" in line
            if "0.95CL" in line:
                assert "1.20 -   0.93 +   0.93" in line
    f.close()
    return True
...
def check_comb_dat(datf):
    f = open(datf)
    for line in f.readlines():
        if line.startswith("a_gaus"):
            assert "1.200197    -0.462758     0.462758" in line
        if line.startswith("b_gaus"):
            assert "2.160118    -0.846813     0.846813" in line
    f.close()
    return True
...
def check_dsets_stdout(logf):
    ...
    for line in f.readlines():
        if (
            line.startswith("branchingRatio = [")
            and "Prob" in line
            and "boundary" not in line
        ):
            if "0.68CL" in line:
                assert "0.00000010 -0.00000011 +0.00000015" in line
            if "0.95CL" in line:
                assert "0.00000010 -0.00000011 +0.00000030" in line
    f.close()
    return True
...
def check_dsets_stdout_cls(logf):
    ...
    for line in f.readlines():
        if (
            ...
            if "0.68CL" in line:
                assert "0.000000103 -0.000000116 +0.000000068" in line
            if "0.95CL" in line:
                assert "0.00000010 -0.00000011 +0.00000025" in line
    f.close()
    return True
...
def check_dsets_dat(datf):
    ...
    for line in f.readlines():
        ...
        if line.startswith("exponent"):
            assert "  -0.000977    -0.000027     0.000027" in line
        if line.startswith("mean"):
            assert "5370.000000     1.000000    -1.000000" in line
        if line.startswith("norm_constant "):
            assert "   0.000000    -0.000000     0.000000" in line
        if line.startswith("norm_constant_sigma"):
            assert "   0.000000     1.000000    -1.000000" in line
        if line.startswith("sigma"):
            assert "  20.900000     1.000000    -1.000000" in line
    f.close()
    return True
...
def write_html_entry(cmd, plot):
    global htmlf
    htmlf.write(
        '<div style="display:inline-block;border-style:solid;border-width:2px;color:black;width:600;word-break:break-all;word-wrap:break-al;">\n'
    )
    htmlf.write("  <center>\n")
    htmlf.write("  </br>\n")
    htmlf.write(
        ' <tt style="border-style:groove;border-color:black;border-width:1px"><mark style="background-color:#D0D3D4">&nbsp;%s&nbsp;</mark></tt> </br>\n'
        % cmd
    )
    htmlf.write(
        '  <a href=\'%s.pdf\'><img width="600" src="%s.png"></a></br>\n' % (plot, plot)
    )
    htmlf.write(
        "  <a href='%s.png'>png</a> &#160; <a href='%s.pdf'>pdf</a>\n" % (plot, plot)
    )
    htmlf.write("  </center>\n")
    htmlf.write("</div>\n")
...
def check_dsets_build_workspace():
    os.system(
        "bin/tutorial_dataset_build_workspace > ci_logs/dsets_build_workspace.log 2>&1"
    )
