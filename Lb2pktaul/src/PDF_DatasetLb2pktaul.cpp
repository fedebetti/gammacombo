#include <PDF_DatasetLb2pktaul.h>
...
PDF_DatasetLb2pktaul::PDF_DatasetLb2pktaul(RooWorkspace* w) : PDF_Datasets(w) {}

RooFitResult* PDF_DatasetLb2pktaul::fit(RooDataSet* dataToFit) {
...
void PDF_DatasetLb2pktaul::generateToys(int SeedShift) {
  TRandom3 rndm(0);

  double mean_signal_events_to_be_generated =
      getWorkspace()->var("branchingRatio")->getVal() / getWorkspace()->var("norm_constant")->getVal();
